...
c  Note that it is an error if any distance is less than 0.2 bohrs.
c
c  ALL REAL VARIABLES ARE DOUBLE PRECISION (REAL*8)
c
c  For a discussion of this surface, see the BKMP2 paper:
c    "A Refined H3 Potential Energy Surface", A.I. Boothroyd,
c    W.J. Keogh, P.G. Martin, and M.R. Peterson, Journal of
c    Chemical Physics, 104, 7139-7152 (1996)
c  and the earlier BKMP paper:
c    "An Improved H3 Potential Energy Surface", A.I. Boothroyd,
c    W.J. Keogh, P.G. Martin, and M.R. Peterson, Journal of
c    Chemical Physics, 95, 4343-4359 (Sept15/1991)
c
c  As of 20 Sept 2003, additional interfaces to this surface have
c  been added, to make it easier to use various coordinate systems
c  (the surface itself remains unchanged, of course).
c
c######################################################################
c
c  r1 is the distance between the first two H atoms
c  r2 is the distance between the second and third H atoms
c  r3 is the distance between the first and third H atoms
c  RR is the distance between the third H atom and the H2 molecule (center)
c  g is the angle between r1 and Rr (in radians)
c  a2, y3, z3 are the minimal cartesian coordinates (that were used
c               when specifying the points fitted by the BKMP2 surface)
c
c                        z-axis          y = y3
c                       ^                |
c                       |                |
...
c######################################################################
c
c  Basic interface, as a function of interatomic distances:
c  ========================================================
c
c  call bkmp2(r,ideriv,Vtot,dVtot)
c  -------------------------------
c
c  Input: r(3) = ( r1, r2, r3 ) , an array contining the three interatomic
c                   distances
c         ideriv = (integer) derivative flag: ideriv > 0 means "calculate
c                     the analytic first derivatives", ideriv = 0 means
c                     "do not do so (return zero for all derivatives)"
c
c  Output: Vtot = total H3 surface energy in hartrees,
c                   relative to separated H + H + H atoms
c          dVtot(3) = ( dVdr1, dVdr2, dVdr3 ) , an array containing the
c                        derivatives of the surface energy with respect
c                        to each interatomic distance (with the other
c                        two distances held fixed)
c
c######################################################################
c
c  Scalar interface, as a function of interatomic distances:
c  =========================================================
c
c  call bkmp2_r(r1,r2,r3,ideriv,Vtot,dVdr1,dVdr2,dVdr3)
c  ----------------------------------------------------
c
c  The same as the basic interface, except that the distnaces and
c  derivatives are given as scalar variables instead of arrays.
c
c######################################################################
c
c  Angular interface:
c  ==================
c
c  call bkmp2_ang(r1,RR,g,ideriv,V,dVdr1,dVdr2,dVdr3,r2,r3,dVdr1gR,dVdRR,dVdg)
c  ---------------------------------------------------------------------------
c
c  Input: r1 = distance between first two H atoms
c         RR = distance between the third H atom and the H2 molecule (center)
c         g = H2 - H angle in radians
c         ideriv = (integer) derivative flag
c
c  Output: V = total H3 surface energy in hartrees
c          dVdr1, dVdr2, dVdr3 = derivatives with respect to the
c                                  three interatomic distnaces
c          r2, r3 = the other two interatomic distances
c          dVdr1gR = derivative of V with respect to r1 at fixed RR and g
c                      (note that this is NOT equal to dVdr1, in general)
c          dVdRR = derivative of V with respect to RR at fixed r1 and g
c          dVdg = derivative of V with respect to g at fixed r1 and RR
c
c######################################################################
c
c  General cartesian interface, including cartesian derivatives:
c  =============================================================
c
c  call bkmp2_cc(c1,c2,c3,ideriv,Vtot,dVdr1,dVdr2,dVdr3,r1,r2,r3,dc1,dc2,dc3)
c  --------------------------------------------------------------------------
c
c  Input: c1(3) = cartesian coordinates (x1,y1,z1) of first H-atom
c         c2(3) = cartesian coordinates (x2,y2,z2) of second H-atom
c         c3(3) = cartesian coordinates (x3,y3,z3) of third H-atom
c         ideriv = (integer) derivative flag
c
c  Output: Vtot = total H3 surface energy in hartrees
c          dVdr1, dVdr2, dVdr3 = derivatives with respect to the
c                                  three interatomic distnaces
c          r1, r2, r3 = the three interatomic distances
c          dc1(3) = derivatives (dVdx1,dVdy1,dVdz1) of V with respect to
c                    cartesian coordinates of first H-atom
c          dc2(3) = derivatives (dVdx2,dVdy2,dVdz2) of V with respect to
c                    cartesian coordinates of second H-atom
c          dc3(3) = derivatives (dVdx3,dVdy3,dVdz3) of V with respect to
c                    cartesian coordinates of third H-atom
c
c  Note: dVdx1,...,dVdz3 are the derivatives with all the other eight
c        cartesian coordinates held fixed in each case.
c
c######################################################################
c
c  Minimal cartesian interface (useful mainly for fitted geometries):
c  ==================================================================
c
c  call bkmp2_cc3(a2,y3,z3,ideriv,Vtot,dVdr1,dVdr2,dVdr3,r1,r2,r3)
c  ---------------------------------------------------------------
c
c  Input: a2 = 0.5 * r1 = position on z-axis of the second H atom
c         y = y3 = distance of third H atom from z-axis
c         z = z3 = height of third H atom above x-y plane
c         ideriv = (integer) derivative flag
c
c  Output: Vtot = total H3 surface energy in hartrees
c          dVdr1, dVdr2, dVdr3 = derivatives with respect to the
...
c
      ...
      subroutine bkmp2_cc(ccH1,ccH2,ccH3,ideriv,V,dVdr12,dVdr23,dVdr13,
     $                    r12,r23,r13,dVdccH1,dVdccH2,dVdccH3)
c     -----------------------------------------------------------------
c
      implicit double precision (a-h,o-z)
c
      dimension ccH1(3), ccH2(3), ccH3(3),
     $     dVdccH1(3), dVdccH2(3), dVdccH3(3)
c
c__Get the three distances as a function of the coordinates.
c
      r12 = sqrt( ( ccH1(1) - ccH2(1) )**2
     $     + ( ccH1(2) - ccH2(2) )**2 + ( ccH1(3) - ccH2(3) )**2 )
      r23 = sqrt( ( ccH2(1) - ccH3(1) )**2
     $     + ( ccH2(2) - ccH3(2) )**2 + ( ccH2(3) - ccH3(3) )**2 )
      r13 = sqrt( ( ccH1(1) - ccH3(1) )**2
     $     + ( ccH1(2) - ccH3(2) )**2 + ( ccH1(3) - ccH3(3) )**2 )
c
      call bkmp2_r(r12,r23,r13,ideriv,Vtot,dVdr12,dVdr23,dVdr13)
c
c__Get the derivatives as a function of the cartesian coordinates.
c
      if ( ideriv .gt. 0 ) then
c
         dVdccH1(1) = dVdr12 * ( ccH1(1) - ccH2(1) ) / r12
     $        + dVdr13 * ( ccH1(1) - ccH3(1) ) / r13
         dVdccH1(2) = dVdr12 * ( ccH1(2) - ccH2(2) ) / r12
     $        + dVdr13 * ( ccH1(2) - ccH3(2) ) / r13
         dVdccH1(3) = dVdr12 * ( ccH1(3) - ccH2(3) ) / r12
     $        + dVdr13 * ( ccH1(3) - ccH3(3) ) / r13
         dVdccH2(1) = - dVdr12 * ( ccH1(1) - ccH2(1) ) / r12
     $        + dVdr23 * ( ccH2(1) - ccH3(1) ) / r23
         dVdccH2(2) = - dVdr12 * ( ccH1(2) - ccH2(2) ) / r12
     $        + dVdr23 * ( ccH2(2) - ccH3(2) ) / r23
         dVdccH2(3) = - dVdr12 * ( ccH1(3) - ccH2(3) ) / r12
     $        + dVdr23 * ( ccH2(3) - ccH3(3) ) / r23
         dVdccH3(1) = - dVdr13 * ( ccH1(1) - ccH3(1) ) / r13
     $        - dVdr23 * ( ccH2(1) - ccH3(1) ) / r23
         dVdccH3(2) = - dVdr13 * ( ccH1(2) - ccH3(2) ) / r13
     $        - dVdr23 * ( ccH2(2) - ccH3(2) ) / r23
         dVdccH3(3) = - dVdr13 * ( ccH1(3) - ccH3(3) ) / r13
     $        - dVdr23 * ( ccH2(3) - ccH3(3) ) / r23
c
      else
c
...
c
      subroutine bkmp2_cc3(a2,y3,z3,ideriv,Vtot,
     $                     dVdr12,dVdr23,dVdr13,r12,r23,r13)
c-----------------------------------------------------------------
c
      implicit double precision (a-h,o-z)
c
c__Get the three distances.
c
      r12 = 2.d0 * a2
      r23 = sqrt( ( z3 - a2 )**2 + y3**2 )
      r13 = sqrt( ( z3 + a2 )**2 + y3**2 )
c
      call bkmp2_r(r12,r23,r13,ideriv,Vtot,dVdr12,dVdr23,dVdr13)
c
c__Derivatives as a function of the minimal cartesian coordinates
c  would not usually be useful; do not compute them.
c
      return
      end
c
      subroutine bkmp2_ang(rhh,rh2h,gammarad,ideriv,Vtot,dVdrhh,
     $                     dVdr2,dVdr3,r2,r3,dVdrhhgR,dVdrh2h,dVdgam)
c-----------------------------------------------------------------
c
      implicit double precision (a-h,o-z)
c
...
c
c__Get the distances r() from rhh, rh2h, and the angle gammarad
c
      cgam = cos(gammarad)
      sgam = sin(gammarad)
      cgamR = cgam * rh2h
      sgamR = sgam * rh2h
      r(1) = rhh
      r(2) = sqrt( ( cgamR - 0.5d0 * r(1) )**2 + sgamR**2 )
      r(3) = sqrt( ( cgamR + 0.5d0 * r(1) )**2 + sgamR**2 )
c
      call bkmp2(r,Vtot,dVtot,ideriv)
c
      dVdrhh = dVtot(1)
      dVdr2 = dVtot(2)
      dVdr3 = dVtot(3)
c
c__Get the derivatives as a function of the angular coordinates.
c
      if ( ideriv .gt. 0 ) then
c
         dr2_drhh = -0.5d0 * ( cgamR - 0.5d0 * r(1) ) / r(2)
         dr3_drhh = 0.5d0 * ( cgamR + 0.5d0 * r(1) ) / r(3)
         dr2_drh2h = ( cgam * ( cgamR - 0.5d0 * r(1) )
     $        + sgam * sgamR ) / r(2)
         dr3_drh2h = ( cgam * ( cgamR + 0.5d0 * r(1) )
     $        + sgam * sgamR ) / r(3)
         dr2_dgam = ( cgamR * sgamR
     $        - sgamR * ( cgamR - 0.5d0 * r(1) ) ) / r(2)
         dr3_dgam = ( cgamR * sgamR
     $        - sgamR * ( cgamR + 0.5d0 * r(1) ) ) / r(3)
c
         dVdrgR = dVtot(1) + dVtot(2) * dr2_drhh + dVtot(3) * dr3_drhh
         dVdrh2h = dVtot(2) * dr2_drh2h + dVtot(3) * dr3_drh2h
         dVdgam = dVtot(2) * dr2_dgam + dVtot(3) * dr3_dgam
c
      else
c
         dVdrgR = 0.d0
...
c
      subroutine bkmp2_r(r1,r2,r3,ideriv,Vtot,dVdr1,dVdr2,dVdr3)
c-----------------------------------------------------------------
c
      implicit double precision (a-h,o-z)
c
...
c  Note that it is an error if any distance is less than 0.2 bohrs.
c
c Note: this file contains parameters for the 'BKMP2' H3 surface,
c       [re]fitted on june21/95 to a set of several thousand ab
c       initio points.  The '706' surface parameters have been
c       commented out (these were the parameters used in the
c       previous 'BKMP' surface).  The routine names have been
c       modified slightly (usually a '95' appended) so that a
c       program could easily call and compare both of our surfaces.
c
...
Cf2py intent(in) r,id
Cf2py intent(out) Vtot,dVtot
